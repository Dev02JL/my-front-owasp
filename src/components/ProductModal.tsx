...
import { fetchAuthenticated } from '@/services/apiClient';
...
const ProductModal = ({ product: initialProduct, onClose }: ProductModalProps) => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      setIsLoading(true);
      try {
        const detailedProduct = await fetchAuthenticated(`/products/${initialProduct.id}`);
        setProduct(detailedProduct);
      } catch (error) {
        console.error("Failed to fetch product details:", error);
        // Keep initial product data on error
                  ...
                  {product.reviews && product.reviews.length > 0 ? (
                    product.reviews.map(review => (
                      <div key={review.id} className="border-b border-gray-700 pb-2">
                        <p className="font-semibold text-green-400">{review.user ? review.user.email : 'Utilisateur anonyme'}</p>
